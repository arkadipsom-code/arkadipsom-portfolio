...
export default function ExperiencesView() {
  return (
    <div className="space-y-10 animate-in fade-in slide-in-from-bottom-4 duration-300">
      ...
      <div>
        <h2 className="text-xl font-bold tracking-widest text-gray-800 uppercase">Work History</h2>
      </div>
        ...
        {experiencesData.map((exp) => (
          <div key={exp.id} className="grid grid-cols-1 sm:grid-cols-4 gap-y-3 gap-x-8">
            ...
            <div className="sm:col-span-1 pt-1">
              <span className="text-xs font-bold text-gray-900 tracking-wider block uppercase">
                {exp.duration}
              </span>
              ...
              {exp.tags && (
                <div className="flex flex-wrap gap-2 pt-2">
                  {exp.tags.map((tag, tagIdx) => (
                    <span key={tagIdx} className="text-xs font-medium text-gray-600 bg-gray-50 px-2.5 py-1 rounded border border-gray-200/60">
                      {tag}
                    </span>
                  ))}
                </div>
